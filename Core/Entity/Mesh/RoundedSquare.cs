...
namespace Xirface
{
    public partial class Mesh
    {
        ...
        public static Mesh<VertexPositionColor> RoundedSquare(Vector2 size, Color fillColor, float curve, float stroke, Color strokeColor, int iterations)
        {
            ...
            size.X = Math.Abs(size.X) * Sign(size.X);
            size.Y = Math.Abs(size.Y) * Sign(size.Y);

            bottomLeft.Outer = new Vector2(-curve * Sign(size.X), -curve * Sign(size.Y));
            bottomLeft.Inner = bottomLeft.Outer + new Vector2(curve * Sign(size.X), curve * Sign(size.Y));
            ...
            topLeft.Outer = new Vector2(bottomLeft.Outer.X, size.Y + curve * Sign(size.Y));
            topLeft.Inner = topLeft.Outer + new Vector2(curve * Sign(size.X), -curve * Sign(size.Y));
            ...
            topRight.Outer = new Vector2(size.X + curve * Sign(size.X), size.Y + curve * Sign(size.Y));
            topRight.Inner = topRight.Outer + new Vector2(-curve * Sign(size.X), -curve * Sign(size.Y));
            ...
            bottomRight.Outer = new Vector2(size.X + curve * Sign(size.X), bottomLeft.Outer.Y);
            bottomRight.Inner = bottomRight.Outer + new Vector2(-curve * Sign(size.X), curve * Sign(size.Y));
